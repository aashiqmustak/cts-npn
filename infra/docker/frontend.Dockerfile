...
RUN sed -i 's/deb.debian.org/mirrors.kernel.org/g' /etc/apt/sources.list.d/debian.sources 2>/dev/null || sed -i 's/deb.debian.org/mirrors.kernel.org/g' /etc/apt/sources.list 2>/dev/null || true
