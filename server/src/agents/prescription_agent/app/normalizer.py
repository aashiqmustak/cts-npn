...
from .config import FREQUENCY_MAPPINGS, ROUTE_MAPPINGS
...
class TextPreprocessor:
    ...
    def preprocess(text: str) -> str:
        ...
        cleaned = re.sub(
            r"(\d+)(mg|mcg|ml|g|unt|units|iu)\b", r"\1 \2", cleaned, flags=re.IGNORECASE
        )

        # Clean unit slash spacing (e.g., mg / ml -> mg/mL)
        cleaned = re.sub(
            r"(\d+)\s*(mg|mcg|unt|units)\s*/\s*(\d*\.?\d*\s*ml)",
            r"\1 \2/\3",
            cleaned,
            flags=re.IGNORECASE,
        )
...
class StrengthExtractor:
    @staticmethod
    def extract(text: str, default_strength: str | None = None) -> str | None:
        if not text:
            return default_strength

        # 0. Explicit strength tag e.g. '(strength 25 mg)' or 'strength: 500 mg/10 mL'
        m_tag = re.search(
            r"strength\s*:?\s*([0-9\.\-\/]+\s*(?:mg|mcg|units|unit|unt|ml|iu|mg\/ml|mcg\/actuation|mg\-mg)(?:\s*\/\s*[0-9\.\s]*(?:ml|iu|actuation))?)",
            text,
            flags=re.IGNORECASE,
        )
        if m_tag:
        ...
        m_multi = re.search(
            r"\b(\d+(?:\.\d+)?\s*mg\s*/\s*\d+(?:\.\d+)?\s*iu)\b",
            text,
            flags=re.IGNORECASE,
        )
        if m_multi:
            return (
                re.sub(r"\s+", " ", m_multi.group(1))
                .replace("iu", "IU")
                .replace("IU", "IU")
                .replace("mg", "mg")
            )
        ...
        m_liquid = re.search(
            r"\b(\d+(?:\.\d+)?\s*(?:mg|mcg|units|unt)\s*/\s*\d*(?:\.\d+)?\s*ml)\b",
            text,
            flags=re.IGNORECASE,
        )
        if m_liquid:
        ...
        m_actuat = re.search(
            r"\b(\d+(?:\.\d+)?\s*(?:mcg|mg)\s*/\s*(?:actuation|actuat))\b",
            text,
            flags=re.IGNORECASE,
        )
        if m_actuat:
        ...
        m_comb = re.search(
            r"(?<!take\s)(?<!sig:\s)\b(\d+(?:\.\d+)?\s*-\s*\d+(?:\.\d+)?\s*(?:mg|mcg))\b(?!\s*/\s*(?:kg|m2|m\^2))",
            text,
            flags=re.IGNORECASE,
        )
        if m_comb:
        ...
        m_single = re.findall(
            r"(?<!-)\b(\d+(?:\.\d+)?)\s*(mg|mcg|g|iu|units|unit)\b(?!-)(?!\s*/\s*(?:kg|m2|m\^2|ml|actuat))",
            text,
            flags=re.IGNORECASE,
        )
        if m_single:
...
class DoseExtractor:
    ...
    def extract(
        ...
        m_m2 = re.search(
            r"(\d+(?:\.\d+)?(?:\s*-\s*\d+(?:\.\d+)?)?\s*mg\s*/\s*(?:m2|m\^2|kg))",
            text,
            flags=re.IGNORECASE,
        )
        if m_m2:
        ...
        m_units = re.search(
            r"\b(\d+)\s*(?:units|unit|unt)\b(?!\s*/\s*ml)", text, flags=re.IGNORECASE
        )
        if m_units:
        ...
        all_doses = re.findall(
            r"(?<!-)\b(\d+(?:\.\d+)?\s*(?:mg|mcg))\b(?!-)(?!\s*/\s*(?:ml|actuat|kg|m2))",
            text,
            flags=re.IGNORECASE,
        )
        if (
            len(all_doses) >= 2
            and extracted_strength
            and all_doses[0].lower() == extracted_strength.lower()
        ):
...
class FrequencyNormalizer:
    @staticmethod
    def normalize(text: str, default_frequency: str | None = None) -> str | None:
        if not text:
...
class RouteNormalizer:
    @staticmethod
    def normalize(text: str, default_route: str | None = None) -> str | None:
        if not text:
        ...
        if re.search(r"\b(intravenous|iv|i\.v\.|infusion|iv drip|iv push)\b", lower):
            return "intravenous"
        if re.search(
            r"\b(oral|po|p\.o\.|by mouth|per os|swallow|sublingual|buccal|oral tablet|oral capsule)\b",
            lower,
        ):
            return "oral"
...
class DurationExtractor:
    @staticmethod
    def extract(text: str, default_duration: int | None = None) -> int | None:
        if not text:
            return default_duration

        m_days = re.search(
            r"(?:for|x|supply|duration)?\s*(\d+)\s*(?:days|day|d)\b",
            text,
            flags=re.IGNORECASE,
        )
        if m_days:
        ...
        m_weeks = re.search(
            r"(?:for|x)?\s*(\d+)\s*(?:weeks|week|wks|wk)\b", text, flags=re.IGNORECASE
        )
        if m_weeks:
            try:
        ...
        m_months = re.search(
            r"(?:for|x)?\s*(\d+)\s*(?:months|month|mo|mos)\b", text, flags=re.IGNORECASE
        )
        if m_months:
            try:
...
class IndicationExtractor:
    @staticmethod
    def extract(text: str, default_indication: str | None = None) -> str | None:
        if not text:
        ...
        m_tag = re.search(
            r"(?:indicated\s+for|indication\s*:?|indic\s*:?|dx\s*:?|diagnosis\s*:?|for\s+(?:the\s+)?treatment\s+of|to\s+treat)\s+([a-zA-Z0-9\s/\-]+?)"
            + stop_pattern,
            text,
            flags=re.IGNORECASE,
        )
        ...
        m_dur_for = re.search(
            r"for\s+duration\s+of\s+\d+\s*(?:days?|weeks?|months?)\s+for\s+([a-zA-Z0-9\s/\-]+?)"
            + stop_pattern,
            text,
            flags=re.IGNORECASE,
        )
        ...
        for m in re.finditer(
            r"\bfor\s+([a-zA-Z\s/\-]+?)" + stop_pattern, text, flags=re.IGNORECASE
        ):
            cand = m.group(1).strip()
            cand = re.sub(r"\s+", " ", cand).strip(" -/,")
            # Exclude duration and dosage form keywords unless part of recognized clinical phrases
            if not re.search(
                r"\b(?:duration|days?|weeks?|months?|oral|daily|bedtime|month|week|day|use)\b",
                cand,
                flags=re.IGNORECASE,
            ):
                if len(cand) > 2 and cand.lower() not in {
